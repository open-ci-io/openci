...
# Cloud Hypervisor VM Startup Script (Direct Kernel Boot)
# Uses Linux kernel directly instead of UEFI firmware - faster and simpler

# Configuration
VM_NAME="${1:-ubuntu-vm}"
ROOT_IMAGE="${2:-ubuntu-cloudhypervisor.raw}"
CLOUDINIT_IMAGE="${3:-cloud-init.img}"
KERNEL="${4:-vmlinux-6.1.bin}"
CPUS="${CH_CPUS:-4}"
...
check_prerequisites() {
    ...
    if [ ! -f "$KERNEL" ]; then
        ...
        log_error "Run: ./download-kernel.sh"
        exit 1
    fi
...
# Start VM
start_vm() {
    log_step "Starting Cloud Hypervisor VM: $VM_NAME (Direct Kernel Boot)"
    ...
    log_info "  Boot: Direct kernel (no UEFI)"
    ...
    # Start Cloud Hypervisor with direct kernel boot
    cloud-hypervisor \
...
usage() {
    cat << EOF
Usage: $0 [VM_NAME] [ROOT_IMAGE] [CLOUDINIT_IMAGE] [KERNEL]

This script uses direct kernel boot (faster than UEFI firmware boot)
...
Examples:
  ...
  # Custom VM name
  ./start-vm-direct.sh my-vm

  # Custom configuration
  CH_CPUS=8 CH_MEMORY=4096M ./start-vm-direct.sh my-vm

  # Background mode
  CH_BACKGROUND=1 ./start-vm-direct.sh
...
main() {
    ...
    log_info "Cloud Hypervisor VM Launcher (Direct Kernel Boot)"
    log_info ""
